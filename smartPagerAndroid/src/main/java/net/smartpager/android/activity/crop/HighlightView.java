...
class HighlightView {
    ...
    private int growDirection=GROW_NONE;
    ...
    protected void draw(Canvas canvas,int parentWidth,int parentHeight) {
        if (mHidden) {
            return;
        }
        canvas.save();
        ...
        if (!hasFocus()) {
           ...
           RectF leftEdge = new RectF(0,0,mDrawRect.left,parentHeight);
            canvas.drawRect(leftEdge,
                    hasFocus() ? mFocusPaint : mNoFocusPaint);
            RectF topEdge = new RectF(mDrawRect.left,0,parentWidth,mDrawRect.top);
            canvas.drawRect(topEdge,
                    hasFocus() ? mFocusPaint : mNoFocusPaint);
            RectF rightEdge = new RectF(mDrawRect.right,mDrawRect.top,parentWidth,parentHeight);
            canvas.drawRect(rightEdge,
                    hasFocus() ? mFocusPaint : mNoFocusPaint);
            RectF buttomEdge = new RectF(mDrawRect.left,mDrawRect.bottom,mDrawRect.left+mDrawRect.width(),parentHeight);
            canvas.drawRect(buttomEdge,
                    hasFocus() ? mFocusPaint : mNoFocusPaint);
            ...
            if (mMode == ModifyMode.Grow) {
                if (mCircle) {
                	...
                	if ((growDirection & GROW_RIGHT_EDGE)!=0)
                		drawRight=true;
                		
                	if ((GROW_BOTTOM_EDGE&growDirection)!=0)
                		drawBottom=true;
                		
                	if ((GROW_LEFT_EDGE&growDirection)!=0)
                		drawLeft=true;
                		
                	if ((GROW_TOP_EDGE&growDirection)!=0)
                		drawTop=true;
    ...
    void growBy(float dx, float dy) {
        ...
        if ((growDirection&GROW_LEFT_EDGE)!=0)
        	if (r.width()>50 || dx>0)
        		r.left-=dx;
        if ((growDirection&GROW_RIGHT_EDGE)!=0)
        	if (r.width()>50 || dx>0)
        		r.right+=dx;
        if ((growDirection&GROW_TOP_EDGE)!=0)
        	if (r.height()>50 || dy>0)
        		r.top-=dy;
        if ((growDirection&GROW_BOTTOM_EDGE)!=0)
        	if (r.height()>50 || dy>0)
        		r.bottom+=dy;
        ...
        if (r.width() < widthCap) {
        	{
        		if ((growDirection&GROW_LEFT_EDGE)!=0)
                	r.left+=-(widthCap - r.width()) / 2F;;
                if ((growDirection&GROW_RIGHT_EDGE)!=0)
                	r.right+=-(widthCap - r.width()) / 2F;;
        		//r.right+=-(widthCap - r.width()) / 2F;
        ...
        if (r.height() < heightCap) {
        	{
        		 if ((growDirection&GROW_TOP_EDGE)!=0)
        	        	r.top+=-(heightCap - r.height()) / 2F;;
        	        if ((growDirection&GROW_BOTTOM_EDGE)!=0)
        	        	r.bottom+=-(heightCap - r.height()) / 2F;;
        		//r.bottom+=-(heightCap - r.height()) / 2F;
        ...
        r.left=Math.max(r.left, mImageRect.left);
        r.right=Math.min(r.right, mImageRect.right);
        r.top=Math.max(r.top, mImageRect.top);
        r.bottom=Math.min(r.bottom, mImageRect.bottom);
    ...
    public void setup(Matrix m, Rect imageRect, RectF cropRect, boolean circle,
        ...
        mFocusPaint.setARGB(140, 0,0,0);
        mNoFocusPaint.setARGB(140,0,0,0);
