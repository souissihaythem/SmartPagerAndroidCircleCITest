...
public class SecretQuestionsActivity extends BaseActivity implements OnDialogDoneListener {
...
//			refreshQuestionsLists(false);
	...
	private void initGui() {
		((TextView) findViewById(R.id.personal_verification_first_title)).setText(getString(
				R.string.settings_verification_question, "1"));
		((TextView) findViewById(R.id.personal_verification_second_title)).setText(getString(
				R.string.settings_verification_question, "2"));
		((TextView) findViewById(R.id.personal_verification_third_title)).setText(getString(
				R.string.settings_verification_question, "3"));
		ArrayAdapter<SecretQuestion> adapter = new ArrayAdapter<SecretQuestion>(this, R.layout.item_spinner_settings,
				m_arrFirstQuestions);
	...
	private boolean validateAnswers() {
		if (TextUtils.isEmpty(m_etFirstAnswer.getText()) || TextUtils.isEmpty(m_etSecondAnswer.getText())
				|| TextUtils.isEmpty(m_etThirdAnswer.getText())) return false;
		return true;
	}
	...
	private void refreshQuestionsLists(boolean isFirstRefresh) {
		if (m_arrAvailableQuestions == null || m_arrAvailableQuestions.size() < 3) return;
		SecretQuestion firstSelectedQuestion = (isFirstRefresh ? m_arrAvailableQuestions.get(0)
				: (SecretQuestion) m_spinnerFirstQuestion.getSelectedItem());
		SecretQuestion secondSelectedQuestion = (isFirstRefresh ? m_arrAvailableQuestions.get(1)
				: (SecretQuestion) m_spinnerSecondQuestion.getSelectedItem());
		SecretQuestion thirdSelectedQuestion = (isFirstRefresh ? m_arrAvailableQuestions.get(2)
				: (SecretQuestion) m_spinnerThirdQuestion.getSelectedItem());
		...
		replaceSelectedQuestion(m_arrFirstQuestions, firstSelectedQuestion,
				m_spinnerFirstQuestion.getSelectedItemPosition());
		replaceSelectedQuestion(m_arrSecondQuestions, secondSelectedQuestion,
				m_spinnerSecondQuestion.getSelectedItemPosition());
		replaceSelectedQuestion(m_arrThirdQuestions, thirdSelectedQuestion,
				m_spinnerThirdQuestion.getSelectedItemPosition());
	...
	private void fillInQuestions(LinkedList<SecretQuestion> destination, LinkedList<SecretQuestion> values) {
		if (values == null || destination == null) return;
		destination.clear();
	...
	protected void onSuccessResponse(WebAction action, AbstractResponse response) {
		...
		if (action == WebAction.setSecretQuestions) {
			...
			SmartPagerApplication.getInstance().startTaskTimer(
					new CancelTimerTask(new PersonalVerificationAlertTimer()));
    ...
    private void isUserLocked() {
        ...
        httpClient.post(this, Constants.BASE_REST_URL + "/getSecretQuestions", params, new JsonHttpResponseHandler() {

            @Override
            public void onSuccess(int statusCode, Header[] headers, JSONObject response) {
                System.out.println("***** successful response: " + response);

                if (response.optString("errorMessage").equalsIgnoreCase("Do lock")) {
                    hideProgressDialog();
                    new AlertDialog.Builder(SecretQuestionsActivity.this)
                            .setTitle("Account Locked")
                            .setMessage("Your account is locked.  Please contact your administrator to continue.")
                            .setNegativeButton("OK", new DialogInterface.OnClickListener() {
                                public void onClick(DialogInterface dialog, int which) {
                                    SecretQuestionsActivity.this.finish();
                                    System.exit(0);
                                }
                            })
                            .show();
